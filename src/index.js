...
class Board extends React.Component {
    ...
    handleClick(i, j) {
        ...
        this.setState({ //изменение состояния игры
            ...
            winner: calculateWinner(squares, i, j, this.state.winCondition), //проверка на победителя
...
//проверка победителя
function calculateWinner(squares, i, j, winCondition) {
    let winner = null;
    ...
    for (let k = 0; winner == null && k < directions.length; k++) {
        winner = checkWinDirection(squares, i, j, directions[k][0], directions[k][1], winCondition);
    }
    ...
    function checkWinDirection(squares, i, j, d1, d2, winCondition) {
        let score = 1;
